...
func showWelcome(api *maxbot.Api, chatID int64) {
	text := "Добро пожаловать в клинику «Смарт Медикал Центр» 👋 \n\n" +
		"Чтобы продолжить запись, необходимо Ваше согласие на обработку персональных данных.\n\n" +
		consentText + "\n\n" +
		"Выберите вариант:\n" +
...
func savePhone(api *maxbot.Api, chatID int64, userID int64, text string) {
	...
	sendText(api, chatID, "Спасибо! Ваш номер сохранён. Сейчас с вами свяжется менеджер.\nВы также можете написать ему напрямую: https://max.ru/u/f9LHodD0cOK3COAqztzCYuQOgnq7p80Y40oFwUMaN9oIFKSHhIv27AQmWNg")
}

func denyConsent(api *maxbot.Api, chatID int64) {
	sendText(api, chatID,
		"К сожалению, без согласия на обработку персональных данных мы не можем продолжить запись в клинику «Смарт Медикал Центр», но Вы всегда можете записаться через колл-центр +74996776088")
}
